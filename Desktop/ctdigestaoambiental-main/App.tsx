...
const App: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const [elec, water, waste, settings, users] = await Promise.all([
        api.getElectricity(),
        api.getWater(),
        api.getWaste(),
        api.getSettings(),
      ...
      setData(prev => ({
        ...
        electricityGoal: settings.electricityGoal || 0,
        waterGoal: settings.waterGoal || 0,
        wasteGoal: settings.wasteGoal || 0,
        users: users
      }));
    } catch (error) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message === 'Invalid credentials') {
        ...
        setLoginError('Erro no servidor/Login: ' + (error.message || 'Erro desconhecido'));
      }
    }
  };
  ...
  const handleImport = (type: keyof AppState, newData: any[]) => {
    // Not implemented strictly for DB yet as import logic is complex
    // For now, let's just alert strictly or iterate
    alert("Importação em massa requer implementação adicional no backend.");
  };

  // --- Unit Management ---
  const handleAddUnit = async () => {
    if (newUnitName.trim() && !data.units.includes(newUnitName.trim())) {
      const newUnits = [...data.units, newUnitName.trim()];
      try {
        await api.updateSettings({ units: newUnits });
        setNewUnitName('');
        await fetchData();
  ...
  const handleDeleteUnit = async (unitName: string) => {
    if (window.confirm(`Tem certeza que deseja remover a unidade "${unitName}"?`)) {
      const newUnits = data.units.filter(u => u !== unitName);
      try {
        await api.updateSettings({ units: newUnits });
        await fetchData();
  ...
  // --- Goal Updates ---
  const handleUpdateElectricityGoal = async (newGoal: number) => {
    try {
      await api.updateSettings({ electricityGoal: newGoal });
      await fetchData();
  ...
  const handleUpdateWaterGoal = async (newGoal: number) => {
    try {
      await api.updateSettings({ waterGoal: newGoal });
      await fetchData();
    } catch (e) { console.error(e); }
  };

  const handleUpdateWasteGoal = async (newGoal: number) => {
    try {
      await api.updateSettings({ wasteGoal: newGoal });
      await fetchData();
    } catch (e) { console.error(e); }
  };

  // --- Data CRUD Handlers (Water, Elec, Waste) ---
  const handleUpdateWater = async (updatedRecord: WaterRecord) => {
    try {
      await api.updateWater(updatedRecord.id, updatedRecord);
      await fetchData();
    } catch (e) { alert("Erro ao atualizar registro."); }
  };

  const handleAddWater = async (newRecord: WaterRecord) => {
    try {
      await api.createWater(newRecord);
      await fetchData();
  ...
  const handleUpdateElectricity = async (updatedRecord: ElectricityRecord) => {
    try {
      await api.updateElectricity(updatedRecord.id, updatedRecord);
      await fetchData();
    } catch (e) { alert("Erro ao atualizar registro."); }
  };

  const handleAddElectricity = async (newRecord: ElectricityRecord) => {
    try {
      await api.createElectricity(newRecord);
      await fetchData();
    } catch (e) { alert("Erro ao adicionar registro."); }
  };

  const handleDeleteElectricity = async (id: string) => {
    if (confirm("Deseja excluir?")) {
      try {
  ...
  const handleUpdateWaste = async (updatedRecord: WasteRecord) => {
    try {
      await api.updateWaste(updatedRecord.id, updatedRecord);
      await fetchData();
    } catch (e) { alert("Erro ao atualizar registro."); }
  };

  const handleAddWaste = async (newRecord: WasteRecord) => {
    try {
      await api.createWaste(newRecord);
      await fetchData();
    } catch (e) { alert("Erro ao adicionar registro."); }
  };
  ...
  return (
    ...
    >
      ...
      <AIChat 
        activeModule={activeModule} 
        moduleData={activeModule === 'electricity' ? data.electricity : activeModule === 'water' ? data.water : data.waste}
        theme={theme}
      />
    </Layout>
  );
