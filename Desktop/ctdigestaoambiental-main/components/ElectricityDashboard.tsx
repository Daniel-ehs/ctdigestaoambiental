...
import { Zap, Leaf, DollarSign, Activity, Sparkles, Plus, Edit2, Trash2, X, Table, Save, Search, Filter, Coins, Camera, Download, Loader2, ChevronDown, Check } from 'lucide-react';
import { ElectricityRecord, UserRole } from '../types';
import { MetricCard } from './Metrics';
import { generateInsights } from '../services/geminiService';
import html2canvas from 'html2canvas';
import MultiSelect from './MultiSelect';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceLine, Cell, AreaChart, Area, LabelList } from 'recharts';
...
export const ElectricityDashboard: React.FC<Props> = ({ data, units, goal, onUpdate, onAdd, onDelete, userRole, theme }) => {
  ...
  const handleAnalyze = async () => {
    ...
    const result = await generateInsights(context, `Energia Elétrica para ${selectedUnits.length === 0 ? 'Todas as Unidades' : selectedUnits.join(', ')}`);
    setInsights(result);
  ...
  const handleExportDashboard = async () => {
    if (dashboardRef.current) {
      ...
      try {
        ...
        const link = document.createElement('a');
        link.download = `Relatorio_Energia_${selectedUnits.length === 0 ? 'Todas' : selectedUnits.join('_')}_${selectedYear}.png`;
            ...
            <BarChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
              ...
              >
                <LabelList 
                  ...
                  formatter={(val: number) => `${new Intl.NumberFormat('pt-BR').format(val)} kWh`}
                />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
            ...
            <AreaChart data={savingsData}>
              ...
              <Area type="monotone" dataKey="savings" name="Economia" stroke="#eab308" strokeWidth={3} fill="url(#colorSavings)">
                <LabelList 
                  dataKey="savings" 
                  position="top" 
                  style={{ fill: '#eab308', fontSize: '10px', fontWeight: 'bold' }}
