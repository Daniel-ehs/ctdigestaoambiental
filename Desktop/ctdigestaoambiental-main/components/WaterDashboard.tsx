...
import { Droplets, DollarSign, Sparkles, TrendingUp, AlertTriangle, Building2, Edit2, Plus, Trash2, X, Save, Table, Search, Filter, Camera, Download, Loader2, ChevronDown, Check } from 'lucide-react';
import { WaterRecord, UserRole } from '../types';
import { MetricCard } from './Metrics';
import { generateInsights } from '../services/geminiService';
import html2canvas from 'html2canvas';
import MultiSelect from './MultiSelect';
import { XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, ComposedChart, Line, Area, AreaChart, Cell, ReferenceLine, LabelList } from 'recharts';
...
const WaterDashboard: React.FC<Props> = ({ data, units, goal, onUpdate, onAdd, onDelete, userRole, theme }) => {
  ...
  const calculatedGoal = useMemo(() => {
    if (selectedUnits.length === 1) return goal;
    if (selectedUnits.length > 1) return goal * selectedUnits.length;
    return goal * units.length;
  }, [goal, selectedUnits, units]);
  ...
  }, [data, selectedUnits, selectedYear]);
  ...
  }, [filteredData, selectedUnits, units]);

  const handleAnalyze = async () => {
    setLoadingInsights(true);
    const context = JSON.stringify(chartData.map(c => ({ m: c.month, v: c.volume, c: c.cost })));
    const result = await generateInsights(context, `Consumo de Água para ${selectedUnits.length === 0 ? 'Todas as Unidades' : selectedUnits.join(', ')}. Atenção à média mensal de ${stats.avgMonthlyVol.toFixed(1)}m³ versus meta de ${GOAL_LIMIT}m³.`);
    setInsights(result);
  ...
  const handleExportDashboard = async () => {
    if (dashboardRef.current) {
      ...
      try {
        ...
        const link = document.createElement('a');
        link.download = `Relatorio_Agua_${selectedUnits.length === 0 ? 'Todas' : selectedUnits.join('_')}_${selectedYear}.png`;
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {userRole === 'manager' && (
            <button
              ...
              className="no-export p-2.5 bg-slate-800 dark:bg-slate-700 text-white rounded-xl shadow-lg shadow-slate-200 dark:shadow-none hover:bg-slate-700 dark:hover:bg-slate-600 hover:shadow-xl transition-all font-medium text-sm"
              ...
              <BarChart data={unitBreakdown} layout="vertical" margin={{ left: 0, right: 30 }}>
                ...
                <Bar dataKey="volume" radius={[0, 4, 4, 0]} barSize={24}>
                  <LabelList 
                    ...
                    formatter={(val: number) => `${new Intl.NumberFormat('pt-BR').format(val)} m³`}
                  />
