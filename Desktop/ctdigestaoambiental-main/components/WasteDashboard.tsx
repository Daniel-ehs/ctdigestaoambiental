...
import { Trash2, Recycle, DollarSign, Sparkles, Leaf, Edit2, Plus, X, Table, Search, Save, Filter, Camera, Download, Loader2, ChevronDown, Check } from 'lucide-react';
import { WasteRecord, UserRole } from '../types';
import { MetricCard } from './Metrics';
import { generateInsights } from '../services/geminiService';
import html2canvas from 'html2canvas';
import MultiSelect from './MultiSelect';
import { PieChart, Pie, Cell, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, AreaChart, Area, ReferenceLine, Sector, LabelList } from 'recharts';
...
const WasteDashboard: React.FC<Props> = ({ data, goal, onUpdate, onAdd, onDelete, userRole, theme }) => {
  ...
  const filteredData = useMemo(() => {
    return data.filter(d => {
      const matchUnit = selectedUnits.length === 0 || selectedUnits.includes(d.unit);
      const matchYear = d.date.startsWith(selectedYear);
      const matchType = selectedType === 'All' || d.type === selectedType;
      return matchUnit && matchYear && matchType;
    });
  }, [data, selectedUnits, selectedYear, selectedType]);
  ...
  const handleAnalyze = async () => {
    ...
    const result = await generateInsights(context, `Gestão de Resíduos (Filtro: ${selectedType === 'All' ? 'Geral' : selectedType}, Unidades: ${selectedUnits.length === 0 ? 'Todas' : selectedUnits.join(', ')}) para o ano ${selectedYear}`);
    setInsights(result);
    ...
    <div ref={dashboardRef} className="space-y-6 animate-fade-in relative pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
        ...
        <div className="flex flex-wrap gap-3">
          <MultiSelect 
            label="Unidades"
            options={[...new Set(data.map(d => d.unit))].sort()}
          ...
          {userRole === 'manager' && (
            <button
              ...
              className="no-export p-2.5 bg-slate-800 dark:bg-slate-700 text-white rounded-xl shadow-lg hover:bg-slate-700 dark:hover:bg-slate-600 transition-all font-medium text-sm"
          ...
          <button
            ...
            className="no-export p-2.5 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600 rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all font-medium text-sm disabled:opacity-50"
              ...
              <BarChart data={monthlyChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                {Object.keys(TYPE_COLORS).map((type, index) => (
                  <Bar
                    ...
                    strokeWidth={1}
                  >
                    {index === Object.keys(TYPE_COLORS).length - 1 && (
                      <LabelList 
                        dataKey={(d: any) => {
                          let total = 0;
                          Object.keys(TYPE_COLORS).forEach(t => total += (d[t] || 0));
                          return total;
                        }}
                        position="top" 
                        style={{ fill: theme === 'dark' ? '#94a3b8' : '#64748b', fontSize: '10px', fontWeight: 'bold' }}
                        formatter={(val: number) => `${new Intl.NumberFormat('pt-BR').format(val)} kg`}
                      />
                    )}
                  </Bar>
                ))}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="totalFinancial" name="Total (R$)" stroke="#8b5cf6" strokeWidth={3} fill="url(#colorFinancial)">
                  <LabelList 
                    dataKey="totalFinancial" 
                    position="top" 
                    style={{ fill: '#8b5cf6', fontSize: '10px', fontWeight: 'bold' }}
                    formatter={(val: number) => `R$ ${new Intl.NumberFormat('pt-BR').format(val)}`}
                  />
                </Area>
              </AreaChart>
            </ResponsiveContainer>
                ...
                <div className="mb-6 bg-white dark:bg-slate-800 p-6 rounded-xl border border-blue-200 dark:border-slate-600 shadow-sm animate-fade-in-up">
                  <h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider mb-4 border-b border-slate-100 dark:border-slate-700 pb-2">{editingId === 'NEW' ? 'Novo Registro' : 'Editando Registro'}</h3>
                   <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
                    ...
                    <div><label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Unidade</label><input type="text" value={formData.unit || ''} onChange={e => setFormData({ ...formData, unit: e.target.value })} placeholder="Ex: Galpão 6" className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-ctdi-blue outline-none" /></div>
