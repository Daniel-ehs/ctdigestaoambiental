...
<!-- Barra superior -->
<div class="top-bar">
    <h1 class="h2 m-0" style="color: #333333;">Cadastro de Locais</h1>
    <a href="index.php?route=cadastros&type=locais&action=create" class="btn btn-success" title="Adicionar Novo Local" style="--btn-index: 1;">
...
<div class="card shadow mb-4">
    <div class="card-header">
        <h6 class="m-0 font-weight-bold">Lista de Locais</h6>
    </div>
    <div class="card-body">
        <table class="table" id="locaisTable">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Nome</th>
            ...
            <tbody>
                ...
                ?>
                <?php if (!empty($dados)): ?>
                    <?php foreach ($dados as $index => $local): ?>
                        <tr style="animation: fadeIn 0.6s ease-out forwards; animation-delay: <?php echo (0.1 * $index); ?>s;" data-id="<?php echo $local['id']; ?>">
                            <td><?= htmlspecialchars($local['id']) ?></td>
                            <td><?= htmlspecialchars($local['nome']) ?></td>
                            <td><?= htmlspecialchars($local['empresa_nome'] ?? 'Não vinculado') ?></td>
                            <td><?= htmlspecialchars($local['nome_setor'] ?? 'N/A') ?></td>
                            <td><?= htmlspecialchars($local['descricao'] ?? '') ?></td>
                            <td><?= $local['ativo'] ? 'Sim' : 'Não' ?></td>
                            <td class="actions-column">
                                <div class="btn-group">
                                    <a href="index.php?route=cadastros&type=locais&action=edit&id=<?= $local['id'] ?>" class="btn btn-sm btn-primary text-white" title="Editar Local" style="--btn-index: 1;">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="index.php?route=cadastros&type=locais&action=delete&id=<?= $local['id'] ?>&name=<?= urlencode($local['nome']) ?>" class="btn btn-sm btn-danger" title="Excluir Local" style="--btn-index: 2;">
