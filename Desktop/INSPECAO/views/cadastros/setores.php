...
<!-- Barra superior -->
<div class="top-bar">
    <h1 class="h2 m-0" style="color: #333333;">Cadastro de Setores</h1>
    <a href="index.php?route=cadastros&type=setores&action=create" class="btn btn-success" title="Adicionar Novo Setor" style="--btn-index: 1;">
...
<div class="card shadow mb-4">
    <div class="card-header">
        <h6 class="m-0 font-weight-bold">Lista de Setores</h6>
    </div>
    <div class="card-body">
        <table class="table" id="setoresTable">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Nome</th>
            ...
            <tbody>
                ...
                ?>
                <?php if (!empty($dados)): ?>
                    <?php foreach ($dados as $index => $setor): ?>
                        <tr style="animation: fadeIn 0.6s ease-out forwards; animation-delay: <?php echo (0.1 * $index); ?>s;" data-id="<?php echo $setor['id']; ?>">
                            <td><?= htmlspecialchars($setor['id']) ?></td>
                            <td><?= htmlspecialchars($setor['nome']) ?></td>
                            <td><?= htmlspecialchars($setor['empresa_nome'] ?? 'Não vinculado') ?></td>
                            <td><?= htmlspecialchars($setor['descricao'] ?? '') ?></td>
                            <td><?= $setor['ativo'] ? 'Sim' : 'Não' ?></td>
                            <td class="actions-column">
                                <div class="btn-group">
                                    <a href="index.php?route=cadastros&type=setores&action=edit&id=<?= $setor['id'] ?>" class="btn btn-sm btn-primary text-white" title="Editar Setor" style="--btn-index: 1;">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="index.php?route=cadastros&type=setores&action=delete&id=<?= $setor['id'] ?>&name=<?= urlencode($setor['nome']) ?>" class="btn btn-sm btn-danger" title="Excluir Setor" style="--btn-index: 2;">
