...
<!-- Barra superior -->
<div class="top-bar">
    <h1 class="h2 m-0" style="color: #333333;">Cadastro de Usuários</h1>
    <a href="index.php?route=cadastros&type=usuarios&action=create" class="btn btn-success" title="Adicionar Novo Usuário" style="--btn-index: 1;">
    ...
    <div class="card-body">
        <table class="table" id="usersTable">
            ...
            <tbody>
                ...
                ?>
                <?php if (!empty($dados)): ?>
                    <?php foreach ($dados as $index => $usuario): ?>
                        <tr style="animation: fadeIn 0.6s ease-out forwards; animation-delay: <?php echo (0.1 * $index); ?>s;" data-id="<?php echo $usuario['id']; ?>">
                            <td><?= htmlspecialchars($usuario['id']) ?></td>
                            <td><?= htmlspecialchars($usuario['nome']) ?></td>
                            <td><?= htmlspecialchars($usuario['email']) ?></td>
                            <td><?= htmlspecialchars($usuario['nivel_acesso']) ?></td>
                            <td><?= htmlspecialchars($usuario['data_criacao']) ?></td>
                            <td><?= htmlspecialchars($usuario['ultimo_acesso'] ?? 'Nunca') ?></td>
                            <td class="actions-column">
                                <div class="btn-group">
                                    <a href="index.php?route=cadastros&type=usuarios&action=edit&id=<?= $usuario['id'] ?>" class="btn btn-sm btn-primary text-white" title="Editar Usuário" style="--btn-index: 1;">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="index.php?route=cadastros&type=usuarios&action=delete&id=<?= $usuario['id'] ?>&name=<?= urlencode($usuario['nome']) ?>" class="btn btn-sm btn-danger" title="Excluir Usuário" style="--btn-index: 2;">
